...
interface Props {
  ...
  initialData?: EfetivoAtual | null;
}

export function EfetivoForm({ isOpen, onClose, onSubmit, initialData }: Props) {
  const [formData, setFormData] = useState<Partial<EfetivoAtual>>({
    postoGrad: 'SD PM',
    quadro: 'QOPM',
    pastaFisica: 'NÃO',
    pastaDigital: 'NÃO',
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="p-6 pb-0">
          <DialogTitle>{initialData ? 'Editar Efetivo' : 'Adicionar Efetivo'}</DialogTitle>
        </DialogHeader>
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
          <form id="efetivo-form" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <DialogFooter className="p-6 pt-0">
          ...
          <Button type="submit" form="efetivo-form">Salvar</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
